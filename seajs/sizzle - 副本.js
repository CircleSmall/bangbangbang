...
var i,
	...
	push = arr.push,
	slice = arr.slice,//从已有的数组中返回指定的元素
	...
	//关系选择器($1:([>+~]|whitespace)分别捕获4种连接符:'>','+','~','whitespace')
...
// Optimize for push.apply( _, NodeList )  优化push
try {
	push.apply(
...
function Sizzle( selector, context, results, seed ) {
	...
	if ( documentIsHTML && !seed ) {
		...
		// Shortcuts  先判断简单的： 类、id、tag
		if ( (match = rquickExpr.exec( selector )) ) {
			// Speed-up: Sizzle("#ID")
			if ( (m = match[1]) ) {
				if ( nodeType === 9 ) {
					elem = context.getElementById( m ); //直接通过原生方法找到id
 ...
 */
 //在最近文档的基础上，这是文档相关的变量，参数是一个元素或者一个文档对象，最终返回一个文档
setDocument = Sizzle.setDocument = function( node ) {
	var hasCompare,
		doc = node ? node.ownerDocument || node : preferredDoc,//当node为document时，用node本身，其他用document
		parent = doc.defaultView;//返回当前document对象所关联的window对象

	// If no document and documentElement is available, return
	if ( doc === document || doc.nodeType !== 9 || !doc.documentElement ) {
		// 初始化时document还是undefined，所以不会执行到这里
		return document;
	}

	// Set our document
	document = doc;//设置document
	docElem = doc.documentElement;//得到所有的元素
	// Support tests
	documentIsHTML = !isXML( doc );//是html
	...
	// 检测getElementsByTagName是否支持注释节点，最好不要支持
	...
	if ( support.getById ) {
		Expr.find["ID"] = function( id, context ) {
			if ( typeof context.getElementById !== "undefined" && documentIsHTML ) {
				...
				// 返回一个数组
				 //!!!!在黑莓4.6的浏览器中会返回那些不在DOM树中的节点，
				 // 所以通过该节点是否有父节点来判断该节点是否在DOM树中,bug修正，并且转换为数组形式。
	...
	if ( (support.qsa = rnative.test( doc.querySelectorAll )) ) {
		...
		assert(function( div ) {
			...
			//!!!空白字符串不应该跟在 ^=、$=、*=这样的字符后面，否则逻辑上是走不通的，你想想啊，^=''的意思是匹配开头为空字符的字符串
			// 。。哪个字符串开头是空字符？，剩下同理
...
//执行matcher, 若参数是多个函数，则把多个函数放到一起执行
...
//把传入的tokens拆分，把每一个token元素对应的编译函数放到一起，并返回
function matcherFromTokens( tokens ) {
	...
	// 遍历tokens,把每个token对应的编译函数放到matchers里
	for ( ; i < len; i++ ) {
		console.log(i)
			...
			for ( ; i !== len && (elem = elems[i]) != null; i++ ) {
				...
				if ( byElement && elem ) {
					...
					while ( (matcher = elementMatchers[j++]) ) {
						if ( matcher( elem, context, xml ) ) {
							// 把每个elem都与编译函数执行一遍，要是返回结果为true,则elem为要选中的节点
							results.push( elem );
							break;
						}
					}
...
compile = Sizzle.compile = function( selector, match /* Internal Use Only */ ) {
	...
	if ( !cached ) {
		...
		i = match.length;//从后开始生成匹配器(这个match是经过初始化后的，即里面已经删除了至少一个用于初步筛选的元素)
		while ( i-- ) {
