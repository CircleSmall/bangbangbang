...
(function(global, undefined) {
  ...
  */
  ...
  function id2Uri(id, refUri) {
    ...
    id = parseAlias(id)   //解析别名
    id = parsePaths(id)
    id = parseAlias(id)
    id = parseVars(id)    //解析变量
    id = parseAlias(id)
    id = normalize(id)    //具体的文件路径
    id = parseAlias(id)

    var uri = addBase(id, refUri)   //获得真正具体的路径
    uri = parseAlias(uri)
    uri = parseMap(uri)
  ...
  if (isWebWorker) {
    ...
    var doc = document
    var scripts = doc.scripts   // 获得页面中所有script标签
  ...
  function getCurrentScript() {
    ...
    // interactive: 语法解析完毕，正在loading替代资源 
  ...
  Module.prototype.load = function() {
    ...
    for (var requestUri in requestCache) {
      if (requestCache.hasOwnProperty(requestUri)) {
        requestCache[requestUri]()      //执行require语句
      }
    }
  }
  ...
  // Execute a module
  // 执行模块factory(即define(fn)的fn)，且将require关键字映射exec上，依次执行所有依赖模块
